...
window.onload = async function () {
  ...
  let request1 = Response.prototype
  try {
  request1 = await fetch("https://api.ipify.org?format=json", {
    method: "GET",
  });
} catch (err) {
  alert("Error al obtener la direccion ip. Por favor desactiva tu AdBlocker");
  console.error("[Game] Error al obtener la direccion ip del usuario: ", err);
  console.warn("[Game] Se detecto un error inesperado. Recargando la pagina...");
  setTimeout(() => {
    window.location.reload();
  }, 3000);
}
  const data = await request1.json();
  console.log("[Game] Obteniendo IP del usuario:" + data.ip);
  ...
  const interval = setInterval(function () {
    ...
    if (clicks_per_second !== Infinity) {
      document.getElementById("clicks_per_second").innerHTML =
        clicks_per_second;
    } else {
    ...
    if (is_active == false) {
      if (clicks_in_sconds <= 0) {
        if (seconds_afk >= 120) {
          ...
          console.warn(
            "[Game] El jugador ha estado afk por mas de 120 segundos, cerrando la sesion"
          );
          console.warn(
            "[Game] Se detecto una terminacion del juego, eliminando variables"
          );
          ...
          const request = await sendPoints(clicks, FinallyID, alias, wins, required, contador, data.ip);
          ...
          setInterval(() => {
            seconds_timeout--;
            console.warn(
              `[Game] Cerrando el juego en ${seconds_timeout} segundos...`
            );
          }, 1000);
          setTimeout(() => {
          ...
          console.warn(
            "[Game] El jugador ha estado afk por mas de 60 segundos, preparando para cerra la sesion"
          );
          document.getElementById("boton").innerHTML = "Estas AFK";
        } else if (seconds_afk >= 30) {
          console.warn(
            "[Game] Iniciando sistema anti afk, segundos afk: " + seconds_afk
          );
        }
      }
    } else {
  ...
  const interval3 = setInterval(function () {
    seconds_afk++;
    console.debug(
      "[Game] Contador de segundos afk: " + seconds_afk,
      "Button is active in the ofther 5 seconds: " + is_active
    );
  }, 1000);
    ...
    if (!IsEnd) {
      if (wins <= 14) {
        if (points >= required) {
          ...
          document.getElementById("required").innerHTML = required;
          console.log(
            "[Game] Se detecto una victoria, TOTAL DE VICTORIAS: " + wins
          );
        } else {
        ...
        document.getElementById("button2").onclick = async function () {
          ...
          setInterval(() => {
            seconds_timeout--;
            console.warn(
              `[Game] Reiniciando el juego en ${seconds_timeout} segundos...`
            );
          }, 1000);
          setTimeout(() => {
      ...
      //Esto va a ser en caso de que se haya decicido continuar sin recompensas
      if (points >= required) {
        required = required * 2;
        wins++;
        document.getElementById("victorias").innerHTML = wins;
  ...
  window.onerror = async function (err) {
    ...
    try {
      ...
      console.warn("[Game] Se obtuvo el alias del jugador: " + alias);
      console.error(
        `[Game] No se pudo eliminar las variables ${error}, forzando reinicio del juego`
      );
      console.warn("[Game] Enviando datos al servidor...");      
      const request = await sendPoints(clicks, FinallyID, alias, wins, required, contador, data.ip);
